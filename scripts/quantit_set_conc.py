...
class QunatiT():
    ...
    def _formated_result_files_dict(self):
        ...
        for f_name, udf_name in file_names.items():
            if self.file_handler.shared_files.has_key(f_name):
                ...
                result_files_dict[udf_name] = self.file_handler.format_file(result_file, 
    ...
    def get_and_set_fluor_int(self, target_file):
        ...
        fluor_int = []
        target_udfs = target_file.udf.items()
        if dict(target_udfs).has_key("Fluorescence intentisy 1"): 
            del target_udfs["Fluorescence intentisy 1"]
        if dict(target_udfs).has_key("Fluorescence intentisy 2"): 
